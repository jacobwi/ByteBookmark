...
import { BookmarkTagDto } from '@bookmarks/shared/models';
import { bookmarkService } from '@bookmarks/shared/services';
import { Button, DataTable, Dialog, Modal } from '@bookmarks/shared/ui';
...
const TagsPage = () => {
  ...
  const handleEditTag = (tag: SetStateAction<BookmarkTagDto | null>) => {
    setEditingTag(tag);
    setIsModalOpen(true);
  };

  const handleSaveTag = async (tag: BookmarkTagDto) => {
    if (tag.id) {
      // Update tag logic
  ...
  const handleDeleteTag = (tagId: SetStateAction<number | null>) => {
